...
- **E-posta + sipariş no toplama:** ziyaretçinin `email`'i yoksa widget'ta sohbetin üstünde engellemeyen kart ("Share your order & email") gösterilir; **sipariş no (opsiyonel)** üstte, e-posta (zorunlu, doğrulanır) altta. `identify` `{email, orderNo?, name?}` alır → `Visitor.email`/`orderNo` kaydeder, operatöre `visitor` event publish eder, kart kaybolur. İsim yalnızca operatör önceden atamadıysa güncellenir. Sipariş no operatör detayında "Order #" satırında görünür ve aramada (`/api/chat/search`) ziyaretçi alanları arasında aranır.
